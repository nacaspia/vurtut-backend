...
{{--    <script src="{{ asset("site/js/maps.js") }}"></script>--}}
    ...
    {{--<script>
        document.addEventListener("DOMContentLoaded", function () {
            axios.get("{{ route('site.mapAjax') }}") // backenddən məlumat al
                .then(function (response) {
                    const companies = response.data.flatMap(category => category.map_companies);
                    const currentLang = "{{ $currentLang }}"; // Blade-dən ötürülən dəyişən
                    const authUserId = {{ auth('user')->check() ? auth('user')->user()->id : 'null' }};
                    let html = '';

                    companies.forEach(company => {
                        const isLiked = company.userLikes && company.userLikes.user_id == {{ auth('user')->user()->id ?? 'null' }};
                        const reviewCount = company.comments?.length ?? 0;

                        html += `
                    <div class="col-lg-12">
                        <div class="feat_property" style="min-height: 404px;max-height: 404px;">
                            <div class="thumb">
                                <img class="img-whp" style="width: 100%; max-height: 164px; object-fit: contain; border-radius: 8px; background-color: #f9f9f9;" src="/uploads/company/${company.image}" alt="${company.full_name}">
                                <div class="thmb_cntnt">
                                    <ul class="tag mb0">
                                        ${company.is_premium == 1 ? `<li class="list-inline-item"><a href="/company/${company.slug}">PEMIUM</a></li>` : ''}
                                    </ul>
                                    <ul class="listing_reviews">
                                        ${renderStars(5)}
                                        <li class="list-inline-item"><a class="text-white total_review" href="/company/${company.slug}">(${reviewCount} Rəy)</a></li>
                                    </ul>
                                </div>
                            </div>
                            <div class="details">
                                <div class="tc_content">
                                    <div class="badge_icon"><a href="/company/${company.slug}"><img src="/site/images/icons/agent1.svg" alt="agent1"></a></div>
                                    <h4>${company.full_name}</h4>
                                    <p>${truncate(company.text, 50)}</p>
                                    <ul class="prop_details mb0">
                                        <li class="list-inline-item"><a href="tel:${company.phone}"><span class="flaticon-phone pr5"></span> ${company.phone}</a></li>
                                        <li class="list-inline-item"><a href="#"><span class="flaticon-pin pr5"></span> ${company.data.address}</a></li>
                                    </ul>
                                </div>
                                <div class="fp_footer">
                                        ...
                                        ${company.category?.image ? `
                                            <li class="list-inline-item">
                                                <a href="/company/${company.slug}">
                                                    <img src="/uploads/categories/${company.category.image}" alt="${company.category.title[currentLang]}">
                                                </a>
                                            </li>` : ''}
                                        <li class="list-inline-item"><a href="/company/${company.slug}">${company.category.title[currentLang]}</a></li>
                                    </ul>
                                    <ul class="fp_meta float-right mb0">
                                        <li class="list-inline-item"><a href="/company/${company.slug}"><span class="flaticon-zoom"></span></a></li>
                                        ${authUserId ? renderLikeButton(company.id, isLiked) : ''}
                                    </ul>
                                </div>
                            </div>
                        </div>
            ...
            function renderStars(count) {
                ...
                for (let i = 0; i < count; i++) {
                    stars += `<li class="list-inline-item"><a class="text-white" href="#"><span class="fa fa-star"></span></a></li>`;
                }
                return stars;
            }
            ...
            function renderLikeButton(id, isLiked) {
                return `
            <li class="list-inline-item">
                <a href="javascript:void(0);" class="like-btn icon"
                   ...
                   data-liked="${isLiked}">
                    <span class="flaticon-love ${isLiked ? 'active' : ''}"></span>
                </a>
            </li>
        `;
            }
